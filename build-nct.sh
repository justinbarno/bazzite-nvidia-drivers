...
DIST_DIR="$(pwd)/build/NVT" \
    make -C ./nvidia-container-toolkit/third_party/libnvidia-container centos8-$ARCH
LIB_NAME="nct" DIST_DIR="$(pwd)/build/NVT" \
    make -C ./nvidia-container-toolkit LIB_TAG="" DOCKER=podman LIB_VERSION=$LIB_VERSION centos8-$ARCH
